...
rule strip_fastq:
    ...
    """
    input:
        lambda w: HTTP.remote(sample_data[w.sample]["fastq"][w.read], username=os.environ["DCC_API_KEY"], password=os.environ["DCC_SECRET_KEY"], keep_local=config["keep_inputs"], allow_redirects=True) 
    output:
