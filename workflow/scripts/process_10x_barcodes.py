...
af = snakemake.input['fastq1']
bf = snakemake.input['fastq_bc']
cf = snakemake.input['fastq2']
barcodesfilepath = snakemake.input['bc_whitelist']
    ...
    with gzip.open(bf, "rt") as f2:
            with gzip.open(cf, "rt") as f3:
                ...
                for i, batch1 in enumerate(it1):
                    ...
                    pm = pool.starmap(debarcode_trio, zip(zip(batch1, batch2, batch3), [max_barcode_dist]*len(batch1)))
                    pool.close()
                    ...
                    filename1 = fastq1_out_path
                    filename2 = fastq2_out_path
        
                    pool = Pool(processes=2)
                    toke = pool.starmap(chunk_writer_gzip, [(filename1, fastq1), (filename2, fastq2)])
                    pool.close()

write_qc(edit_dists, qc_path)
